...
app.get('/api/query/:table', async (req, res) => {
  try {
    ...
    if (search) {
      // Data query parameters: $1=limit, $2=offset, $3=search
      whereClauseData = `WHERE CAST(ten_khuon AS TEXT) ILIKE $3`;
      // Count query parameters: $1=search
      whereClauseCount = `WHERE CAST(ten_khuon AS TEXT) ILIKE $1`;
      queryParams.push(`%${search}%`);
    }

    const countResult = await pool.query(`SELECT COUNT(*) as total FROM ${table} ${whereClauseCount}`, search ? [`%${search}%`] : []);
